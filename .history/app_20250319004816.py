...
# Modified function to load and train models with proper model selection
def train_model(dataset_path):
    ...
    is_classifier = unique_values <= 10  # Assume <= 10 unique values means classification
    ...
    return model, scaler, list(X.columns), target_column, is_classifier

# Train models
models, scalers, features, is_classifier = {}, {}, {}, {}
try:
    for disease, path in datasets.items():
        model, scaler, feature_names, target_col, is_clf = train_model(path)
        models[disease] = model
        scalers[disease] = scaler
        features[disease] = feature_names
        is_classifier[disease] = is_clf
...
def predict_disease(disease_key, input_values):
    try:
        ...
        if is_classifier[disease_key]:
            return int(prediction[0])  # Return as int for classification
        else:
            return float(prediction[0])  # Return as float for regression
    except Exception as e:
...
if selected:
    ...
    if st.button(f"🔍 Predict {selected.replace(' Prediction', '')}"):
        if user_inputs:
            ...
            # Display results differently based on classifier/regressor
            if disease_key in is_classifier and is_classifier[disease_key]:
                if result == 1:
                ...
                st.success(f"📊 Prediction value: {result:.2f}")
        else:
