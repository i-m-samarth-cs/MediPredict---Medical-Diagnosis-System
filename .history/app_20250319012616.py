...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("Made with ❤️ by Your Samarth Shendre")
...
# Modified Input Function without nested columns
...
def main():
    ...
    with col1:
        ...
        if disease_key in input_fields:
            # Determine if we should use columns based on number of inputs
            use_columns = len(input_fields[disease_key]) > 5
            
            if use_columns:
                # Create two columns for the entire input section
                left_col, right_col = st.columns(2)
                
                # Split the inputs between the two columns
                half = len(input_fields[disease_key]) // 2
                left_inputs = input_fields[disease_key][:half]
                right_inputs = input_fields[disease_key][half:]
                
                # Process left column inputs
                with left_col:
                    for feat, desc, min_val, max_val in left_inputs:
                        user_inputs.append(display_input(feat, desc, feat, min_val, max_val))
                
                # Process right column inputs
                with right_col:
                    for feat, desc, min_val, max_val in right_inputs:
                        user_inputs.append(display_input(feat, desc, feat, min_val, max_val))
            else:
                # Process inputs in a single column
                for feat, desc, min_val, max_val in input_fields[disease_key]:
                    user_inputs.append(display_input(feat, desc, feat, min_val, max_val))
