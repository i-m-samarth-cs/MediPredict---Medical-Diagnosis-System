...
background_img = """
    <style>
    .stApp {
        background: url("https://wallpaperaccess.com/full/4968293.jpg");
...
disease_info = {
    ...
    "Heart Disease": "A range of heart conditions affecting heart function, commonly due to blocked blood vessels.",
    "Parkinson's": "A progressive nervous system disorder affecting movement, often causing tremors.",
    "Lung Cancer": "A type of cancer that starts in the lungs, usually caused by smoking or pollution.",
    "Hypo-Thyroid": "A condition where the thyroid gland does not produce enough hormones, leading to fatigue and weight gain."
}
...
# Display Section
if selected:
    disease_key = selected.split()[0].lower()
    st.markdown(f"### 🏥 {selected}")
    st.info(disease_info[selected.split()[0]])  # Display disease info

    user_inputs = [display_input(feat, desc, feat) for feat, desc in input_fields[disease_key]]

    if st.button(f"🔍 Predict {selected.split()[0]}"):
        result = predict_disease(disease_key, user_inputs)
        if result == 1:
            st.success(f"⚠️ The person has {selected.split()[0]}!")
        else:
            st.success(f"✅ The person does not have {selected.split()[0]}.")
